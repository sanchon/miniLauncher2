...
Si el fichero no tiene ninguna sección o una sección no define `template`, la carga falla con un error claro.
...
Texto de la **plantilla** que se ejecuta o abre. Puede incluir **marcadores** `{nombre_param}` que se sustituyen por los valores que el usuario pasa en la línea de órdenes.
...
Si pones un `mode` distinto de estos tres, la carga del INI **falla**.
...
Para cada parámetro presente en `values`, se reemplaza `{clave}` en `template`:
...
|-------|-------------|-------------------|
| `template` | Sí | Plantilla con `{param}`. |
| `description` | No | Texto para `--list`. |
| `mode` | No (`shell` por defecto) | `shell` \| `browser` \| `open`. |
